...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            if noisyDistance == None:   # case when ghost is captured; thats when noisyDistance will be none
                bxList[p] = 0
                bxList[self.getJailPosition()] = 1
            else:                       # based on logic explained in slide 63 of hmm document
                bxList[p] = self.beliefs[p] * emissionModel[util.manhattanDistance(p, pacmanPosition)]
        bxList.normalize()
        self.beliefs = bxList
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        bdashxdashList = util.Counter()
        for p in self.legalPositions:
            newDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, p)) # used based on the information given in comments
            # print(newDistribution.items())
            for newPos, prob in newDistribution.items():
                bdashxdashList[newPos] += prob * self.beliefs[p]    # based on slide 63 in hmm document
        self.beliefs = bdashxdashList
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # add items to particles list as many as determined by numParticles.
        index = 0
        self.particles = []
        while index < self.numParticles:
            self.particles += [position for position in self.legalPositions]
            index += len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.particles = [self.getJailPosition() for _ in range(self.numParticles)] # ghost capturing scneario is
            # handled as mentioned in the comment
        else:
            bxList = util.Counter() # based on slide 74 in hmm. B(X) = P(e | X)*B'(X)
            bdashxList = self.getBeliefDistribution()
            for position in self.legalPositions:
                bxList[position] = emissionModel[util.manhattanDistance(position, pacmanPosition)] * bdashxList[
                    position]
            flag = False
            for x in bxList.values():
                if x != 0:
                    flag = True
                    break
            if flag:
                self.particles = [util.sample(bxList) for _ in range(self.numParticles)]
            else:
                self.initializeUniformly(gameState) # case when all particles weight is zero
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        xdashList = []  #based on slide 73 in hmm.x' = sample(P(X'|x))
        for x in self.particles:
            pxdashX = self.getPositionDistribution(self.setGhostPosition(gameState, x))
            xdashList += [util.sample(pxdashX)]
        self.particles = xdashList
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # hint given above to use itertools.product; logic same as applied to ParticleFilter initializeUniformly
        productList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(productList)
        index = 0
        self.particles = []
        while index < self.numParticles:
            self.particles += [position for position in productList]
            index += len(productList)
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        for i in range(self.numGhosts): # as mentioned in the above comments
            if noisyDistances[i] == None:
                self.particles = [self.getParticleWithGhostInJail(p,i) for p in self.particles] # hinted in the comment
            else:
                bxList = util.Counter()
                bdashxList = self.getBeliefDistribution()
                for particle in self.particles:
                    bxList[particle] = emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)] * bdashxList[
                        particle]   # same logic as done in Question 4 for Particle Filtering.
                    # Only change is handling emissionModels(2-D matrix) for multiple ghosts
                flag = False
                for x in bxList.values():
                    if x != 0:
                        flag = True
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            latestParticle = [] #logic same as Question 5 with minor changes
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                             i, self.ghostAgents[i])
                latestParticle += [util.sample(newPosDist)]
            "*** END YOUR CODE HERE ***"
            newParticles.append(tuple(latestParticle))
