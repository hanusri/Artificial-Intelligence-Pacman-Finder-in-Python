...
def enhancedFeatureExtractorDigit(datum):
    ...
    Given, a digital image, the number of connected components of white space varies based on the given digital image.
    The number of connected components of white space is calculated using depth first search on the datum matrix
    Based on this value, the digital image is guessed.
    ##
    """
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if (datum.getPixel(x, y) == 0 and marked[x][y] == 0):
                marked = connectedComponents(datum, x, y, marked)
                regions += 1
    ...
    # print regions
    # feature values are set based on number of connected components of white space.
    # Each digital image has different connected components
    if regions == 1:
        features[1] = 1
        features[2] = 1
        features[3] = 1
...
def connectedComponents(datum, x, y, marked):
    xAxis = [-1, 0, 0, 1]
    yAxis = [0,-1,1,0]
    marked[x][y] = 1
    k = 4
    for i in range(k):
        if isSafe(marked,x+xAxis[i],y+yAxis[i],datum) == True:
            marked = connectedComponents(datum, x + xAxis[i], y + yAxis[i], marked)
    return marked

def isSafe(marked,x,y,datum):
    if x >= 0 and x < DIGIT_DATUM_WIDTH and y >= 0 and y < DIGIT_DATUM_HEIGHT and marked[x][y] == 0 and datum.getPixel(x,y) == 0:
        return True
    else:
        return False
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    # the logic is based on the evaluation function I wrote in project 2. Some of the key factors that was calculated
    # before where moved over here for pacman feature.
    ...
    nearestUnScaredGhost = None
    nearestScaredGhost = None
    for ghostId in range(len(newGhostStates)):
        ...
        if newScaredTimes[ghostId] == 0:
            if nearestUnScaredGhost == None or nearestUnScaredGhost > dist:
                nearestUnScaredGhost = dist
        else:
            if nearestScaredGhost == None or nearestScaredGhost > dist:
                nearestScaredGhost = dist

    if nearestUnScaredGhost != None:
        features['nearestunscaredGhost'] = nearestUnScaredGhost

    if nearestScaredGhost != None:
        features['nearestscaredGhost'] = nearestScaredGhost

    features['foodcount'] = foodcount

    if len(newFood) != 0:
        nearestFood = min([util.manhattanDistance(loc,newPos) for loc in newFood])
        features['nearestFood'] = nearestFood

    features['score'] = successorGameState.getScore()
    return features
...
def getClosestDistance(self, position, objects):
    if len(objects) == 0:
        return 0
    return min([util.manhattanDistance(position, obj) for obj in objects])
